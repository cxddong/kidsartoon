...
router.post('/create', async (req, res) => {
    try {
        ...
        console.log('[CartoonBook API] Create request:', { userId, vibe, totalPages, layout, hasAssets: !!assets });
        ...
        const costActionMap: Record<number, string> = { 4: 'cartoon_book_4', 8: 'cartoon_book_8', 12: 'cartoon_book_12' };
        const action = costActionMap[totalPages];

        console.log(`[CartoonBook API] Deducting points for ${action}...`);
        const deduction = await pointsService.consumePoints(userId, action);

        if (!deduction.success) {
            console.log('[CartoonBook API] Insufficient points');
        ...
        console.log('[CartoonBook API] Points deducted successfully');

        const taskId = await worldBuilderService.createCartoonBook(userId, {
            vibe,
            assets,
        ...
        console.log(`[CartoonBook API] Task created: ${taskId}`);

        res.json({ taskId, status: 'PENDING', cost: totalPages === 4 ? 100 : totalPages === 8 ? 180 : 250 });
    } catch (error: any) {
        console.error('[CartoonBook API] Creation error:', error);
        res.status(500).json({ error: error.message || 'Failed to create cartoon book' });
    }
});
...
router.get('/status/:taskId', async (req, res) => {
    try {
        ...
        const task = await databaseService.getCartoonBookTask(taskId);
        ...
        console.error('[CartoonBook API] Status check error:', error);
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const novel = await databaseService.getCartoonBook(id);
        if (!novel) return res.status(404).json({ error: 'Cartoon book not found or not completed' });
        res.json(novel);
    } catch (error: any) {
        console.error('[CartoonBook API] Fetch error:', error);
        res.status(500).json({ error: error.message || 'Failed to fetch cartoon book' });
    }
});
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const novels = await databaseService.getUserCartoonBooks(userId);
        res.json({ novels });
    } catch (error: any) {
        console.error('[CartoonBook API] User novels fetch error:', error);
        res.status(500).json({ error: error.message || 'Failed to fetch novels' });
    }
});

/**
 * POST /api/cartoon-book/coach-asset
...
router.post('/coach-asset', async (req, res) => {
    try {
        ...
        console.log('[CartoonBook API] Coach asset request:', { userId, role, vibe });
        ...
        console.log('[CartoonBook API] Points deducted, analyzing image...');
        ...
        // ---------------------------------------------------------
        try {
            console.log('[CartoonBook API] Attempt 1: Calling Doubao Vision (Sight)...');
            ...
            console.log('[CartoonBook API] Doubao Success. Calling Gemini Reasoning...');
...
`;
            ...
            console.warn('[CartoonBook API] ⚠️ Hybrid Analysis Failed:', hybridError.message);
            ...
            // ---------------------------------------------------------
            try {
                console.log('[CartoonBook API] Attempt 2: Falling back to Gemini Vision...');
                ...
                console.log('[CartoonBook API] ✅ Gemini Vision Fallback Success');

            } catch (geminiError: any) {
                console.error('[CartoonBook API] ❌ Gemini Vision Failed:', geminiError.message);
                if (geminiError.status === 503) console.error('Gemini Service Unavailable (Overloaded)');

                // ---------------------------------------------------------
                // ATTEMPT 3: MOCK DATA (Final Safety Net)
                // ---------------------------------------------------------
                console.log('[CartoonBook API] Using Mock Data Fallback');
                analysis = {
        ...
        console.log('[CartoonBook API] Final Analysis:', analysis);
        res.json({
        ...
        console.error('[CartoonBook API] Coach asset error:', error);
