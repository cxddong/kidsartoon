...
export const AudioStoryPage: React.FC = () => {
    ...
    const [lang, setLang] = useState<'en' | 'zh' | 'fr' | 'es'>('zh');
    ...
    const handleGenerate = async () => {
        if (!imageFile) return;
        setStep('processing');
        setStatusText("Analyzing your image... (This relies on Gemini Vision)");
    ...
    return (
        <div className="h-screen w-full bg-[#FAFAFA] text-slate-800 flex flex-col font-sans">
            {/* Header */}
            <header className="p-4 flex items-center gap-4 bg-white shadow-sm shrink-0 z-10">
                <button onClick={() => navigate('/generate')} className="p-2 rounded-full hover:bg-slate-100 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto w-full max-w-md mx-auto p-6 flex flex-col items-center">
