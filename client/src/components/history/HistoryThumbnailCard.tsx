...
const HistoryThumbnailCard: React.FC<HistoryThumbnailCardProps> = ({ image, onClick }) => {
    ...
    const getTypeIcon = () => {
        switch (image.type) {
            case 'animation': return <Play size={14} className="fill-current" />;
            case 'comic':
    ...
    const getTypeColor = () => {
        switch (image.type) {
            ...
            case 'comic': return 'bg-orange-500';
    ...
    return (
        ...
        >
            ...
            {/* Type Badge */}
            <div className={`absolute top-2 left-2 px-2 py-1 ${getTypeColor()} text-white text-[10px] font-bold uppercase tracking-wider rounded-lg shadow-sm flex items-center gap-1.5 z-10`}>
                {getTypeIcon()}
                {image.type}
            </div>
