# Script to adjust AnimationPage.tsx layout: move upload to top, mic to text field
import re
...
# Find the params section start
params_start_marker = "                {/* STEP 2: PARAMETERS */}"
params_start = content.find(params_start_marker)

if params_start == -1:
    print("Could not find params section!")
    exit(1)

# Find the section end
params_end_marker = "                {/* STEP 3 & 4: GENERATING & FINISHED */}"
params_end = content.find(params_end_marker)
...
# New params section with upload at top and mic inside textarea
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-4">
                            ...
                            <div className="bg-slate-800/50 rounded-2xl p-3 border-2 border-slate-700/50">
                                <div>
                                    ...
                                    {/* Textarea with Mic Button on Right */}
                                    <div className="relative">
                                        ...
                                        />
                                        {/* Mic Button - Inside Right */}
                                        <button
                                            onClick={() => {
                                                if (!isListening) handleListen();
                                            }}
                                            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 hover:bg-purple-500/20 rounded-lg transition-colors group"
                                            title="Click to Speak"
                                        >
                                            <img src="/mic_icon_3d.png" alt="Mic" className="w-10 h-10 object-contain drop-shadow-lg group-hover:scale-110 transition-transform" />
                                        </button>
                                    </div>
...
print("✅ Upload moved to top! Mic moved to text field right side!")
